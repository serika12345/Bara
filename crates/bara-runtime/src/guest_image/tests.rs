...
#[test]
fn mach_o_executable_mapping_carries_mapped_bytes_value() {
    let image = MachOImage::executable(
        MachOExecutableEntryPoint::new(X86Va::new(0x1_0000_0010)),
        mach_o_code_segment(),
        metadata(),
    ...
    assert_eq!(mapping.mapped_bytes(), &guest_image_mapped_bytes());
    assert_eq!(
        mapping
            .mapped_bytes()
            .payload()
...
#[test]
fn mach_o_image_exposes_executable_metadata_snapshot() {
    let image = MachOImage::executable(
        MachOExecutableEntryPoint::new(X86Va::new(0x1_0000_0010)),
        mach_o_code_segment(),
        metadata(),
    ...
    .expect("entry is inside code segment");
    let metadata = image.executable_metadata();

    assert_eq!(metadata.mapped_bytes(), &guest_image_mapped_bytes());
    assert_eq!(metadata.sections(), &guest_image_sections());
    assert_eq!(metadata.symbols(), &guest_image_symbols());
    assert_eq!(metadata.relocations(), &guest_image_relocations());
    assert_eq!(metadata.imports(), &guest_image_imports());
    assert_eq!(metadata.unwind(), &guest_image_unwind());
}

#[test]
fn mach_o_image_exposes_executable_image_snapshot() {
    let image = MachOImage::executable(
        MachOExecutableEntryPoint::new(X86Va::new(0x1_0000_0010)),
        mach_o_code_segment(),
        metadata(),
    ...
    let expected_mapping = image.executable_mapping();
    let expected_metadata = image.executable_metadata();

    assert_eq!(snapshot.mapping(), &expected_mapping);
    assert_eq!(snapshot.metadata(), &expected_metadata);
    assert_eq!(snapshot.mapping().code_segment(), mach_o_code_segment());
    assert_eq!(snapshot.metadata().imports(), &guest_image_imports());
}
...
#[test]
fn mach_o_executable_image_snapshot_exposes_program_image_metadata_view() {
    let image = MachOImage::executable(
        MachOExecutableEntryPoint::new(X86Va::new(0x1_0000_0010)),
        mach_o_code_segment(),
        metadata(),
    ...
    assert_eq!(
        image.executable_snapshot().program_image_metadata(),
        program_image_metadata()
    );
}
