...
#[test]
fn sub_eax_imm_return_39_decodes_lifts_and_emits_arm64() {
    let test_case = test_case_from_json(include_str!(
        "../../../tests/cases/sub_eax_imm_return_39.json"
    ))
    .expect("sub_eax_imm_return_39 test case parses");
    let input = test_case.x86_bytes().clone();
    let decoded = decode_function(&input).expect("sub eax imm8 bytes decode");
    let program = lift_decoded_function(&decoded).expect("sub eax imm8 bytes lift");
    let emitted = emit_program(&program).expect("sub eax imm8 IR emits");
...
fn sub_eax_imm_return_39_runs_on_supported_aarch64_unix_hosts() -> Result<(), String> {
    ...
    let expected = observed_result_from_json(include_str!(
        "../../../tests/expected/sub_eax_imm_return_39.json"
    ))
    .expect("sub_eax_imm_return_39 expected result parses");
    let input = test_case.x86_bytes().clone();
    let decoded = decode_function(&input).expect("sub eax imm8 bytes decode");
    let program = lift_decoded_function(&decoded).expect("sub eax imm8 bytes lift");
    let emitted = emit_program(&program).expect("sub eax imm8 IR emits");
...
#[test]
fn sub_eax_imm32_return_39_decodes_lifts_and_emits_arm64() {
    let test_case = test_case_from_json(include_str!(
        "../../../tests/cases/sub_eax_imm32_return_39.json"
    ))
    .expect("sub_eax_imm32_return_39 test case parses");
    let input = test_case.x86_bytes().clone();
    let decoded = decode_function(&input).expect("sub eax imm32 bytes decode");
    let program = lift_decoded_function(&decoded).expect("sub eax imm32 bytes lift");
    let emitted = emit_program(&program).expect("sub eax imm32 IR emits");
...
fn sub_eax_imm32_return_39_runs_on_supported_aarch64_unix_hosts() -> Result<(), String> {
    ...
    let expected = observed_result_from_json(include_str!(
        "../../../tests/expected/sub_eax_imm32_return_39.json"
    ))
    .expect("sub_eax_imm32_return_39 expected result parses");
    let input = test_case.x86_bytes().clone();
    let decoded = decode_function(&input).expect("sub eax imm32 bytes decode");
    let program = lift_decoded_function(&decoded).expect("sub eax imm32 bytes lift");
    let emitted = emit_program(&program).expect("sub eax imm32 IR emits");
...
#[test]
fn add_sub_eax_imm_return_40_decodes_lifts_and_emits_arm64() {
    let test_case = test_case_from_json(include_str!(
        "../../../tests/cases/add_sub_eax_imm_return_40.json"
    ))
    .expect("add_sub_eax_imm_return_40 test case parses");
    let input = test_case.x86_bytes().clone();
    let decoded = decode_function(&input).expect("add/sub eax imm8 bytes decode");
    let program = lift_decoded_function(&decoded).expect("add/sub eax imm8 bytes lift");
    let emitted = emit_program(&program).expect("add/sub eax imm8 IR emits");
...
fn add_sub_eax_imm_return_40_runs_on_supported_aarch64_unix_hosts() -> Result<(), String> {
    ...
    let expected = observed_result_from_json(include_str!(
        "../../../tests/expected/add_sub_eax_imm_return_40.json"
    ))
    .expect("add_sub_eax_imm_return_40 expected result parses");
    let input = test_case.x86_bytes().clone();
    let decoded = decode_function(&input).expect("add/sub eax imm8 bytes decode");
    let program = lift_decoded_function(&decoded).expect("add/sub eax imm8 bytes lift");
    let emitted = emit_program(&program).expect("add/sub eax imm8 IR emits");
