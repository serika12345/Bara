...
- [ ] B8 の完了条件を、arm64 macOS 上で実 x86_64 macOS アプリが Bara 経由で起動し、決定的な expected / actual 比較を通すこととする。
  ...
  - [ ] feedback report の `unsupported_loader_feature` に対して、public Mach-O loader metadata から最初の user-space loader 実行計画を作る。
  - [ ] AppKit import / Objective-C runtime boundary を helper boundary または明示 blocker として進め、expected / actual 差分を縮める。
  - [ ] arm64 macOS 上で self-authored x86_64 GUI Hello World が Bara 経由で起動し、Rosetta expected / Bara actual 比較を通す。
