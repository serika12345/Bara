...
- [ ] B8 の完了条件を、arm64 macOS 上で実 x86_64 macOS アプリが Bara 経由で起動し、決定的な expected / actual 比較を通すこととする。
  ...
  - [ ] arm64 macOS 上で self-authored x86_64 GUI Hello World が Bara 経由で起動し、Rosetta expected / Bara actual 比較を通す。
    ...
    - [ ] Objective-C runtime / AppKit lifecycle helper capability の host execution を actual path に接続し、current blocker を解除する。
    - [ ] GUI Hello World actual result を Rosetta expected と一致させる。
