...
- [x] 最初に起動対象とする実 x86_64 macOS アプリの scope と成功条件を定義する。
  ...
  に置く。
- [ ] self-authored single-binary GUI Hello World source を追加し、x86_64
  Mach-O executable としてビルドできる host-specific fixture にする。
- [ ] GUI Hello World fixture を Rosetta black-box oracle で実行し、
  `expected.json` と launch metadata の初期 schema を固定する。
- [ ] Bara 側の GUI Hello World 起動 attempt を `actual.json` / launch
  report / blocker classification として保存できる CLI 境界を作る。
- [ ] GUI Hello World の initial blocker を unsupported import /
