...
### M8: 実 x86_64 macOS アプリ起動

- [ ] 実 x86_64 macOS アプリの最初の起動対象を決める。
- [ ] Mach-O executable image 全体を入力として扱う。
- [ ] user-space loader / runtime の最小境界を実装する。
- [ ] source ISA mode と partial register model を固定しすぎない。
- [ ] unsupported loader feature / import / instruction を stable report にする。
- [ ] 起動、stdout/stderr、exit status を expected / actual で比較する。

### M9: 実 x86 32-bit アプリ対応

- [ ] x86 32-bit アプリ対応の最初の起動対象を決める。
- [ ] blocker が大きい場合は記録し、B10 へ進めるようにする。
- [ ] source ISA mode と 32-bit ABI を domain type で分ける。
- [ ] partial register / address size / operand size を扱う。
- [ ] unsupported 32-bit feature を stable report にする。
