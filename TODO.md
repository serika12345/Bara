...
- [ ] entry、segments、sections、imports、relocations、必要な loader metadata を public Mach-O 仕様ベースで model 化する。
  ...
  - [ ] sections metadata を public `LC_SEGMENT_64` / section table から model 化する。
  - [ ] imports metadata を public dynamic-link load commands から model 化する。
  - [ ] relocations / rebases / binds に必要な loader metadata の扱いを分ける。
