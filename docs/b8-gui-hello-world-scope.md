...
- `input`: Mach-O executable image、source ISA、binary format、target triple、
  ...
  recognized entry points / segments、section table metadata、executable image
  conversion blocker を含む。imports、relocations は public parser が対応するまで
  `not_modeled` として明示する。
...
5. Mach-O loader metadata を public format から段階的に増やす。最初は
   ...
   追加する。sections は `LC_SEGMENT_64` の public section table から model 化する。
