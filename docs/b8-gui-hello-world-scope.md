...
- `runtime_preparation`: `bara-runtime` の user-space launch plan 由来の
  ...
  event、stdout lifecycle observation を planned not executed として記録する。
...
- `feedback_report`: Rosetta expected と Bara actual / launch report を束ねる
  ...
  次の修正対象は `connect_appkit_lifecycle_helper_execution` とする。
- `blocker`: 初期分類、選ばれた boundary、選択規則、候補 boundary、説明。
  現時点では Objective-C runtime helper boundary が最初の未対応 helper boundary
  であるため `unsupported_objc_runtime_boundary` とする。候補分類は
...
22. Objective-C runtime / AppKit lifecycle helper capability contract を
   ...
   と current blocker 解除は後続 step で扱う。
23. helper boundary または runtime support を追加し、Rosetta expected と Bara
