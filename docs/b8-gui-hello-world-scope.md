...
- `runtime_preparation`: `bara-runtime` の user-space launch plan 由来の
  ...
  分ける。`executable_memory` は runtime の責務として、allocation を
  ...
  記録する。`integration_policy` は current user-space process を scope とし、
...
9. executable memory は public OS API (`mmap` / `mprotect` など) 経由に
   ...
   step で扱う。
10. unsupported import、unsupported loader feature、unsupported Objective-C
   runtime boundary を stable classification として report する。
11. helper boundary または runtime support を追加し、expected / actual 比較を
   通す。
