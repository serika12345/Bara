...
- `runtime_preparation`: `bara-runtime` の user-space launch plan 由来の
  ...
  分ける。`source_isa_profile` は x86_64 long mode、address size 64-bit、
...
- `feedback_report`: Rosetta expected と Bara actual / launch report を束ねる
  ...
  current blocker、`loader_execution_plan`、次の修正対象を stable JSON として
  保存する。初期値では `unsupported_loader_feature` を current blocker とし、
  次の修正対象は
...
21. helper boundary または runtime support を追加し、Rosetta expected と Bara
