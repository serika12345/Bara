...
- `runtime_preparation`: `bara-runtime` の user-space launch plan 由来の
  ...
  `not_required` として記録する。現時点では `planned_not_executed` として扱う。
...
7. kernel extension、private kernel hook、private dyld behavior を前提にしない。
   ...
   後続 step で扱う。
8. unsupported import、unsupported loader feature、unsupported Objective-C
   runtime boundary を stable classification として report する。
9. helper boundary または runtime support を追加し、expected / actual 比較を
   通す。
