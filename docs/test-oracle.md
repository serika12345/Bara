...
経路に通し、`compiled.ir.json`、`pcmap.json`、`fixups.json`、`helpers.json` を
指定 directory に保存する。初期 schema は regression 用の stable JSON とし、
fixups は ARM64 branch lowering で適用した source / target / kind を記録する。
