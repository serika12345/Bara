...
- active_branch: `task/b8-g5a-import-helper-marshaling-contract`。base branch は
  `task/b8-g5-import-helper-boundary-request` の `dde0888`
  (`docs: record b8-g5 review gate`) で、B8-G5 draft PR #25 の上に積む stacked
  branch として扱う。latest commit は B8-G5a review package で報告する。
...
- remaining_work: B8-G5b。B8-G5a の helper marshaling contract が出す
  ...
  は行わない。
- next_action: B8-G5a branch を commit / push し、draft PR を開いて review gate で
  停止する。レビュー後の次 PR Gate は B8-G5b ObjC Message Materialization Boundary。
