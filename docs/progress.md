...
1. [TODO.md](../TODO.md) の `B8-ARCH1 Post-HWGUI Responsibility Split Audit`、
   ...
   `B8-ARCH2g B8 Debug Bundle Guest Image Mapping Shell Split`、
   `B8-ARCH2h Runtime GuestImage Domain Shell`、
   `B8-ARCH2i Runtime GuestImage Mapped Bytes Shell`、
   ...
   `B8-ARCH2af Runtime MachO Image Mapping Snapshot`、
...
- active_work: completed。B8-ARCH2am Runtime MachO Program Metadata View を
  ...
  `docs/design-todo.md` の `B8-ARCH2am result`。意図は B8-ARCH2al で snapshot-backed にした
  helper boundary から metadata aggregate の組み立て責務も runtime domain へ寄せること。
  `MachOExecutableImageMetadata::program_image_metadata()` は typed metadata value object 群から
  existing downstream materialization 用の compatibility view を返し、CLI の local assembly
  function は削除した。existing B8 debug bundle behavior と `loader.plan.json` output は維持。
  dependency、schema、import/fixup/symbol semantics、helper bridge、runtime dispatcher は未変更。
  ...
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c cargo fmt --check`、`git diff --check`、
  `nix develop -c ./scripts/verify`。すべて pass。
...
- active_work: completed。B8-ARCH2ak Debug Loader MachO Snapshot Boundary を
  ...
  `docs/design-todo.md` の `B8-ARCH2ak result`。意図は B8 debug bundle loader plan が
  Mach-O executable image snapshot を単一の入口として扱い始め、image mapping projection が
  entry input から image model を作り直さない境界にすること。`btbc-cli` は
  `mach_o_executable_snapshot_from_entry_input` で snapshot を作り、
  `B8DebugLoaderPlanReport::real_lc_main_attempted` はその snapshot を
  `B8DebugGuestImageMappingReport::from_mach_o_snapshot` へ渡す。existing B8 debug bundle
  ...
  import/fixup/symbol projection semantics の意味変更、helper bridge、runtime dispatcher は未移動。
  依存・lockfile・toolchain 変更はない。remaining work は PR review 後に B8-ARCH2 Guest
  Image Model Extraction の次 slice、または helper process / Objective-C bridge 境界の
  TODO-backed PR Gate を選ぶこと。verification は
  `nix develop -c cargo test -p btbc-cli image_mapping_report_uses_mach_o_executable_image_snapshot -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
...
- active_work: completed。B8-ARCH2ai Runtime MachO Executable Metadata Snapshot を
  ...
  `docs/design-todo.md` の `B8-ARCH2ai result`。意図は後続の import / relocation / symbol /
  unwind projection が generic metadata 内部ではなく Mach-O executable image snapshot を入口に
  できるようにし、B8-ARCH2 の Mach-O specific image model 境界を強めること。`bara-runtime` は
  `MachOExecutableImageMetadata` と `MachOImage::executable_metadata()` を追加し、caller が
  Mach-O specific snapshot 経由で mapped bytes / sections / symbols / relocations / imports /
  unwind value object を扱えるようにした。既存の payload accessor は維持し、existing B8
  debug bundle behavior と `loader.plan.json` output は維持。`bara-oracle` からの loader
  domain 抽出、entry extraction / load command interpretation、public Mach-O parser / resolver
  logic、import/fixup/symbol projection semantics の意味変更、helper bridge、runtime dispatcher は
  未移動。依存・lockfile・toolchain 変更はない。remaining work は PR review 後に B8-ARCH2
  Guest Image Model Extraction の次 slice または helper process / Objective-C bridge 境界の
  TODO-backed PR Gate を選ぶこと。verification は
  `nix develop -c cargo test -p bara-runtime mach_o_image_exposes_executable_metadata_snapshot -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2ah Runtime GuestImage Metadata Value Accessors を
  `task/b8-arch2ah-guest-image-metadata-value-accessors` で実施した。関連 TODO は
  `TODO.md` の `B8-ARCH2ah Runtime GuestImage Metadata Value Accessors`、関連設計メモは
  `docs/design-todo.md` の `B8-ARCH2ah result`。意図は後続 runtime loader caller が
  `GuestImageMetadata` から metadata collection を payload primitive ではなく runtime-facing
  value object として受け取れるようにし、B8-ARCH2 の image model 境界を強めること。
  `bara-runtime` は `GuestImageMetadata::sections_value()` / `symbols_value()` /
  `relocations_value()` / `imports_value()` / `unwind_value()` を追加し、mapped bytes /
  sections / symbols / relocations / imports / unwind を同じ value object 境界から扱えるように
  した。既存の payload accessor は維持し、existing B8 debug bundle behavior と
  `loader.plan.json` output は維持。`bara-oracle` からの loader domain 抽出、entry extraction /
  load command interpretation、public Mach-O parser / resolver logic、import/fixup/symbol
  projection semantics の意味変更、helper bridge、runtime dispatcher は未移動。依存・lockfile・
  toolchain 変更はない。remaining work は PR review 後に B8-ARCH2 Guest Image Model
  Extraction の次 slice または helper process / Objective-C bridge 境界の TODO-backed PR Gate を
  選ぶこと。verification は
  `nix develop -c cargo test -p bara-runtime guest_image_metadata_exposes_metadata_value_objects -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2ag Runtime MachO Mapping Mapped Bytes Snapshot を
  `task/b8-arch2ag-macho-mapping-bytes-payload` で実施した。関連 TODO は
  `TODO.md` の `B8-ARCH2ag Runtime MachO Mapping Mapped Bytes Snapshot`、関連設計メモは
  `docs/design-todo.md` の `B8-ARCH2ag result`。意図は B8 debug bundle と後続 runtime loader
  caller が mapping snapshot から mapped bytes source だけでなく payload value object も
  読めるようにし、debug DTO projection の理由で `MachOImage` / metadata 内部構造へ戻らない
  境界を強めること。`bara-runtime` は `MachOExecutableImageMapping` が
  `GuestImageMappedBytes` value object を保持するようにし、
  `MachOExecutableImageMapping::mapped_bytes()` で source と payload を同じ runtime-facing
  snapshot から扱えるようにした。`MachOExecutableImageMapping::mapped_bytes_source()` は残し、
  source は snapshot 内の value object から導出する。`btbc-cli` の
  `B8DebugGuestImageMappingReport` は existing API で mapped bytes source を report し、
  existing B8 debug bundle behavior と `loader.plan.json` output は維持。`bara-oracle` からの
  loader domain 抽出、entry extraction / load command interpretation、public Mach-O parser /
  resolver logic、import/fixup/symbol projection semantics の意味変更、helper bridge、runtime
  dispatcher は未移動。依存・lockfile・toolchain 変更はない。remaining work は PR review 後に
  ...
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2af Runtime MachO Image Mapping Snapshot を
  `task/b8-arch2af-macho-image-mapping-snapshot` で実施した。関連 TODO は
  `TODO.md` の `B8-ARCH2af Runtime MachO Image Mapping Snapshot`、関連設計メモは
  `docs/design-todo.md` の `B8-ARCH2af result`。意図は B8 debug bundle の
  `image_mapping` projection が必要とする code segment、entry point、mapped bytes source を
  `MachOImage` の内部 accessor 群から直接集めるのではなく、runtime domain の snapshot として
  渡すこと。`bara-runtime` は `MachOExecutableImageMapping` を追加し、
  `MachOImage::executable_mapping()` で snapshot を返せるようにした。これにより caller は
  `MachOExecutableImageMapping` を単位に mapping 構成を扱えるようになり、debug DTO
  projection の理由で `MachOImage` の metadata 構造を知る必要がなくなった。`btbc-cli` の
...
- active_work: completed。B8-ARCH2ad Debug Guest Image MachO Projection Boundary を
  ...
  `docs/design-todo.md` の `B8-ARCH2ad result`。意図は B8 debug bundle の
  `image_mapping` projection が runtime の typed `MachOImage` boundary を直接使うようにし、
  generic image invariant の再検証を CLI report DTO 側から外すこと。
  `btbc-cli` は `crates/btbc-cli/src/b8_debug_bundle/guest_image.rs` の
  `B8DebugGuestImageMappingReport::from_entry_input` を `MachOImage` projection 経由に寄せ、
  `MachOImage::code_segment()` の non-optional contract を使うようにした。これにより debug
  report projection から存在しない code segment 欠落 path を読まなくてよくなった。
  `image_mapping_report_uses_typed_mach_o_code_segment` を追加し、typed Mach-O code segment から
  report が作られることを固定した。existing B8 debug bundle behavior と
  `loader.plan.json` output は維持。`bara-oracle` からの loader domain 抽出、
  entry extraction / load command interpretation、public Mach-O parser / resolver logic、
  import/fixup/symbol projection semantics の意味変更、helper bridge、runtime dispatcher は
  未移動。依存・lockfile・toolchain 変更はない。remaining work は PR review 後に
...
- active_work: completed。B8-ARCH2ab Runtime GuestImage Core Module Split を
  ...
  `docs/design-todo.md` の `B8-ARCH2ab result`。意図は generic `GuestImage` shell と親
  `guest_image/mod.rs` の変更理由を分け、parent module を submodule wiring / re-export /
  tests に近づけること。`bara-runtime` は
  `crates/bara-runtime/src/guest_image/image.rs` を追加し、generic `GuestImage` shell、
  `GuestImageEntryPoint`、`GuestImageSegments`、`GuestImageSegment`、
  `GuestImageSegmentKind`、`GuestImageSegmentSource`、`GuestImageAddressSpace`、
  `GuestImageError` を `guest_image/mod.rs` から分けた。これにより generic image invariant、
  entry point、segment identity、image error の調整を `guest_image/image.rs` 側で扱えるように
  なり、metadata / Mach-O specific module と親 module を変更せずに generic image shell を
  見直せる。`guest_image/mod.rs` は generic image module を re-export し、
  `bara_runtime::GuestImage` などの existing public API 名は維持。`metadata.rs` と
  `mach_o.rs` は existing public API / caller-visible behavior を維持したまま generic image shell
  を module boundary 経由で使う。`GuestImage` / `MachOImage` / B8 debug bundle の
  caller-visible behavior と `loader.plan.json` output は維持。`bara-oracle` からの
  loader domain 抽出、entry extraction / load command interpretation、public Mach-O parser /
  resolver logic、import/fixup/symbol projection semantics の意味変更、helper bridge、
  runtime dispatcher は未移動。依存・lockfile・toolchain 変更はない。remaining work は
  PR review 後に B8-ARCH2 Guest Image Model Extraction の次 slice または helper process /
  Objective-C bridge 境界の TODO-backed PR Gate を選ぶこと。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2aa Runtime MachOImage Module Split を
  `task/b8-arch2aa-macho-image-module-split` で実施した。関連 TODO は
  `TODO.md` の `B8-ARCH2aa Runtime MachOImage Module Split`、関連設計メモは
  `docs/design-todo.md` の `B8-ARCH2aa result`。意図は generic `GuestImage` shell と
  Mach-O specific `MachOImage` shell の変更理由を分け、runtime image model の親 module が
  Mach-O constructor / executable code range の詳細で肥大化しないようにすること。
  `bara-runtime` は `crates/bara-runtime/src/guest_image/mach_o.rs` を追加し、
  `MachOImage`、`MachOExecutableEntryPoint`、`MachOExecutableCodeRange`、
  `MachOExecutableCodeSegment` を `guest_image/mod.rs` から分けた。これにより Mach-O
  executable entry / code range / code segment の調整を `guest_image/mach_o.rs` 側で扱えるように
  なり、generic image shell 側を変更せずに Mach-O specific constructor boundary を見直せる。
  `guest_image/mod.rs` は Mach-O module を re-export し、`bara_runtime::MachOImage` などの
  existing public API 名は維持。`GuestImageSegment::mach_o_executable_code` は generic shell から
  外し、Mach-O executable code segment assembly は `MachOExecutableCodeSegment::new` 側へ閉じた。
  `GuestImage` / `MachOImage` / B8 debug bundle の caller-visible behavior と
  `loader.plan.json` output は維持。`bara-oracle` からの loader domain 抽出、
  entry extraction / load command interpretation、public Mach-O parser / resolver logic、
  import/fixup/symbol projection semantics の意味変更、helper bridge、runtime dispatcher は
...
- active_work: completed。B8-ARCH2y Runtime GuestImage Relocations Value Object を
  ...
  semantics を変えずに後続の loader/fixup 境界を扱いやすくすること。`bara-runtime` は
  `GuestImageRelocations` を追加し、`ProgramImageRelocations` payload を runtime-facing
  value object として保持する。これにより `GuestImageMetadata` construction は
  `ProgramImageRelocations` を直接受け取らず、`GuestImageRelocations` を受け取って保持できる。
  ...
  閉じる。`GuestImage` / `GuestImageMetadata` の existing `relocations()` accessor は維持。
  ...
  import/symbol projection の意味変更、helper bridge、runtime dispatcher は未移動。
  依存・lockfile・toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2x Runtime GuestImage Imports Value Object を
  `task/b8-arch2x-guest-image-imports-value` で実施した。意図は imports payload を
  `GuestImageMetadata` の direct collection field から分け、import projection semantics を
  変えずに後続の loader/import 境界を扱いやすくすること。`bara-runtime` は
  `GuestImageImports` を追加し、`ProgramImageImports` payload を runtime-facing value object
  として保持する。これにより `GuestImageMetadata` construction は
  `ProgramImageImports` を直接受け取らず、`GuestImageImports` を受け取って保持できる。
  `GuestImageMetadata::from_program_image_metadata` は
  `GuestImageImports::from_program_image_metadata` 経由で imports clone を value object 側に
  閉じる。`GuestImage` / `GuestImageMetadata` の existing `imports()` accessor は維持。
  B8 debug bundle は existing `GuestImage` projection 経由で existing
  `B8DebugGuestImageMappingReport` へ射影し、`loader.plan.json` の image mapping JSON は維持。
  `bara-oracle` からの loader domain 抽出、entry extraction / load command interpretation、
...
- active_work: completed。B8-ARCH2v Runtime GuestImage Symbols Value Object を
  ...
  import projection 境界を意味変更なしに切り出しやすくすること。`bara-runtime` は
  `GuestImageSymbols` を追加し、`ProgramImageSymbols` payload を runtime-facing value
  object として保持する。これにより `GuestImageMetadata` construction は
  `ProgramImageSymbols` を直接受け取らず、`GuestImageSymbols` を受け取って保持できる。
  `GuestImageMetadata::from_program_image_metadata` は
  `GuestImageSymbols::from_program_image_metadata` 経由で symbols clone を value object 側に
  閉じる。`GuestImage` / `GuestImageMetadata` の existing `symbols()` accessor は維持。
  B8 debug bundle は existing `GuestImage` projection 経由で existing
  `B8DebugGuestImageMappingReport` へ射影し、`loader.plan.json` の image mapping JSON は維持。
  `bara-oracle` からの loader domain 抽出、entry extraction / load command interpretation、
  public Mach-O parser / resolver logic、import/fixup/symbol projection の意味変更、
...
- active_work: completed。B8-ARCH2t Runtime GuestImage Mapped Bytes Value Object を
  ...
  一体の runtime-facing value object として保持する。`GuestImageMetadata` は
  `mapped_bytes_source` と `ProgramImageMappedBytes` を別々に constructor へ受け取らず、
  `GuestImageMappedBytes` を受け取って保持する。`GuestImageMetadata::from_program_image_metadata` は
  `GuestImageMappedBytes::from_program_image_metadata` 経由で source selection と payload clone を
  value object 側に閉じる。`GuestImage` / `GuestImageMetadata` の existing
  `mapped_bytes_source()` と `mapped_bytes()` accessor は維持。B8 debug bundle は existing
  `GuestImage` projection 経由で existing `B8DebugGuestImageMappingReport` へ射影し、
  `loader.plan.json` の image mapping JSON は維持。`bara-oracle` からの loader domain 抽出、
  entry extraction / load command interpretation、public Mach-O parser / resolver logic、
  helper bridge、runtime dispatcher は未移動。依存・lockfile・toolchain 変更はない。
  verification は `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2s Runtime MachO Code Segment Domain Type を
  `task/b8-arch2s-mach-o-code-segment-domain-type` で実施した。`bara-runtime` は
  `MachOExecutableCodeSegment` を追加し、Mach-O executable code segment の range、source、
  address-space を runtime-facing domain type として保持する。`MachOImage` constructor は
  generic `GuestImageSegment` ではなく `MachOExecutableCodeSegment` を受け取り、underlying
  `GuestImageSegment` への変換を constructor 内に閉じる。`MachOImage::code_segment` は
  `Option<GuestImageSegment>` ではなく `MachOExecutableCodeSegment` を返し、valid code segment
  を持つ invariant を API に反映する。B8 debug bundle は existing `GuestImage` projection
  経由で existing `B8DebugGuestImageMappingReport` へ射影し、`loader.plan.json` の image
  mapping JSON は維持。`bara-oracle` からの loader domain 抽出、entry extraction / load
  command interpretation、public Mach-O parser / resolver logic、helper bridge、runtime
  dispatcher は未移動。依存・lockfile・toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
...
- active_work: completed。B8-ARCH2q Runtime MachO Code Section Range Constructor を
  ...
  選ぶ判断を runtime constructor 側に閉じる。missing / ambiguous code section は
  `GuestImageError` として分類する。`MachOImage::executable_from_program_image_metadata` は
  caller-provided `MachOExecutableCodeRange` を受け取らず、entry point と
  `ProgramImageMetadata` だけから image shell を作る。B8 debug bundle は code bytes length
  由来の `ProgramImageRange` を計算せず、existing `B8DebugGuestImageMappingReport` へ射影する。
  `loader.plan.json` の image mapping JSON は維持。`bara-oracle` からの loader domain 抽出、
  public Mach-O parser / resolver logic、helper bridge、runtime dispatcher は未移動。
  依存・lockfile・toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
...
- active_work: completed。B8-ARCH2o Runtime MachOImage Program Metadata Constructor を
  ...
  runtime constructor 側に閉じる。B8 debug bundle は existing `MachOEntryFunctionInput` から
  `ProgramImageRange` と `ProgramImageMetadata` を渡し、existing
  `B8DebugGuestImageMappingReport` へ射影する。`loader.plan.json` の image mapping JSON は維持。
  `bara-oracle` からの loader domain 抽出、helper bridge、runtime dispatcher は未移動。
  依存・lockfile・toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2n Runtime MachOImage Code Range Constructor を
  `task/b8-arch2n-mach-o-code-range-constructor` で実施した。`bara-runtime` は
  `MachOImage::executable_from_code_range` を追加し、Mach-O executable code segment の
  source / address-space 決定を runtime constructor 側に閉じる。B8 debug bundle は
  existing `MachOEntryFunctionInput` から `ProgramImageRange` と `GuestImageMetadata` を作り、
  `MachOImage::executable_from_code_range` を通して existing `B8DebugGuestImageMappingReport`
...
- active_work: completed。B8-ARCH2l Runtime GuestImage Metadata Collections Shell を
  ...
  symbols / relocations / imports / unwind を aggregate 経由で保持する。B8 debug bundle は
  existing `MachOEntryFunctionInput::program_image_metadata()` から `GuestImageMetadata` を
  作って `GuestImage::mach_o_executable` へ渡す。`loader.plan.json` の image mapping JSON は
  維持。`MachOImage` 本体、helper bridge、runtime dispatcher は未移動。依存・lockfile・
  toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2k Runtime GuestImage Relocations Shell を
  `task/b8-arch2k-guest-image-relocations-shell` で実施した。`bara-runtime::GuestImage` は
  `ProgramImageRelocations` を保持し、B8 debug bundle は
  `MachOEntryFunctionInput::program_image_metadata().relocations()` を
  `GuestImage::mach_o_executable` へ渡す。`loader.plan.json` の image mapping JSON は
  維持。`MachOImage` 本体、symbol identity、helper bridge、runtime dispatcher は未移動。
  依存・lockfile・toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2j Runtime GuestImage Imports Shell を
  `task/b8-arch2j-guest-image-imports-shell` で実施した。`bara-runtime::GuestImage` は
  `ProgramImageImports` を保持し、B8 debug bundle は
  `MachOEntryFunctionInput::program_image_metadata().imports()` を
  `GuestImage::mach_o_executable` へ渡す。`loader.plan.json` の image mapping JSON は
  維持。`MachOImage` 本体、fixups/symbol identity、helper bridge、runtime dispatcher は
  未移動。依存・lockfile・toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2i Runtime GuestImage Mapped Bytes Shell を
  `task/b8-arch2i-guest-image-mapped-bytes-shell` で実施した。`bara-runtime::GuestImage` は
  `ProgramImageMappedBytes` を保持し、B8 debug bundle は
  `MachOEntryFunctionInput::program_image_metadata().mapped_bytes()` を
  `GuestImage::mach_o_executable` へ渡す。`loader.plan.json` の image mapping JSON は
  維持。`MachOImage` 本体、imports/fixups/symbol identity、helper bridge、
  runtime dispatcher は未移動。依存・lockfile・toolchain 変更はない。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`。
- active_work: completed。B8-ARCH2h Runtime GuestImage Domain Shell を
  `task/b8-arch2h-guest-image-domain-shell` で実施した。`bara-runtime::guest_image` に
  parser 非依存の `GuestImage` shell を追加し、B8 debug bundle の `image_mapping`
  report は `MachOEntryFunctionInput` からこの shell へ射影してから existing JSON DTO を
  作る。`MachOImage` 本体、imports/fixups/symbol identity、helper bridge、
  runtime dispatcher は未移動。verification は
  `nix develop -c cargo test -p bara-runtime guest_image -- --nocapture`、
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture`、
  `nix develop -c ./scripts/verify`、`nix develop -c ./scripts/verify-supply-chain`。
...
- active_milestone: completed。[TODO.md](../TODO.md) の B8-ARCH2g B8 Debug
  Bundle Guest Image Mapping Shell Split を
  `task/b8-arch2g-debug-guest-image-mapping-shell` 上で進めた。B8-ARCH2f は
  PR #57 で merge 済みであり、B8-ARCH2g では loader plan shell に残っている image
  mapping summary DTO を
  `crates/btbc-cli/src/b8_debug_bundle/guest_image.rs` へ分ける。`loader.plan.json` の
  `image_mapping` field 名、nested field 名、serde 値、JSON output は維持し、import/fixup
  projection、helper process execution、modeled continuation、runtime dispatcher、
  `GuestImage` / `MachOImage` domain model 本体抽出には進まない。
...
- active_branch: `task/b8-arch2g-debug-guest-image-mapping-shell`。branch base は
  `7290fe5`
  (`Merge pull request #57 from serika12345:task/b8-arch2f-debug-helper-boundary-marshaling-split`)。
  この branch は B8-ARCH2g guest image mapping shell split 用であり、GuestImage 本体抽出、
...
- related_todo: [TODO.md](../TODO.md) B8-D0 / B8-G2 / B8-G3 / B8-G3b / B8-G3c /
  ...
  B8-ARCH1 / B8-ARCH1a / B8-ARCH2a / B8-ARCH2b / B8-ARCH2c / B8-ARCH2d /
  B8-ARCH2e / B8-ARCH2f / B8-ARCH2g / B8-ARCH2 / B8-ARCH3 / B8-ARCH4 /
  B8-ARCH5 / B8-ARCH6 / B8-OSS0 / B8-WINE0。
...
- B8-ARCH2b として、`crates/btbc-cli/src/b8_debug_bundle/io.rs` を追加し、bundle
  directory layout、output path JSON、JSON/bin/text file read/write helper、repro
...
- B8-ARCH2d として、`B8DebugLoaderPlanReport` と直接の loader plan shell DTO を
  `crates/btbc-cli/src/b8_debug_bundle/loader.rs` へ分けた。既存 `generate_b8_debug_bundle`
  は loader plan report を作り、`helper_boundary_request()` で launch report へ existing
  helper boundary request を接続する。`loader.plan.json` schema 名、field 名、JSON output は
  維持した。
- B8-ARCH2e として、`B8DebugImportBoundaryReport` と public import metadata projection DTO を
  `crates/btbc-cli/src/b8_debug_bundle/import_boundary.rs` へ分けた。`loader.rs` は
  `B8DebugImportBoundaryReport::from_probe_and_decode_report` を呼び、
...
- B8-ARCH2g として、`B8DebugGuestImageMappingReport` と直接の segment source / address
  space / mapped bytes source enum を
  `crates/btbc-cli/src/b8_debug_bundle/guest_image.rs` へ分けた。`loader.rs` は
  `B8DebugGuestImageMappingReport::from_entry_input` を呼ぶ形にし、`loader.plan.json` の
  `image_mapping` field 名、nested field 名、serde 値、JSON output は維持した。
- remaining_work: B8-ARCH2g review gate。commit / push / draft PR 作成後に停止する。
  B8-ARCH2 Guest Image Model Extraction 本体、
  helper bridge 一般化、translation artifact/cache/dispatcher 実装へはこの branch では進まない。
- next_action: B8-ARCH2g guest image mapping shell split を review する。merge 後は TODO の
  次の PR Gate を追加または選び、B8-ARCH2 Guest Image Model Extraction の次の小さい
  slice、または helper process / Objective-C bridge 境界分割へ進む。
- verification:
...
- 2026-06-14 14:37 JST: B8-ARCH2g B8 Debug Bundle Guest Image Mapping Shell Split を
  code PR Gate として完了状態にした。B8-ARCH2f merge 後の次 slice として、
  loader plan の image mapping summary shell だけを `guest_image.rs` へ分けた。
  import/fixup projection、helper process execution、modeled continuation、runtime
  dispatcher、`GuestImage` / `MachOImage` 本体抽出には進まない。
...
- 2026-06-14 12:21 JST: B8-ARCH2e B8 Debug Bundle Import Boundary Projection Split を開始し、
  ...
  projection DTO だけを `import_boundary.rs` へ分けた。helper request / marshaling、
  helper process execution、modeled continuation state、`GuestImage` / `MachOImage` 本体抽出は
  この PR に含めない。対象テスト
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture` が通過した。
  full verification も通過した。
- 2026-06-14 11:57 JST: B8-ARCH2d B8 Debug Bundle Loader Plan Shell Split を開始し、
  code PR Gate として完了状態にした。
  TODO-backed PR Gate を追加し、`B8DebugLoaderPlanReport` と直接の loader plan shell DTO
  だけを `loader.rs` へ分けた。`loader.plan.json` schema 名、field 名、JSON output は
  維持し、import/fixup projection、helper process execution、modeled continuation state、
  `GuestImage` / `MachOImage` 本体抽出はこの PR に含めない。対象テスト
  `nix develop -c cargo test -p btbc-cli generate_b8_debug_bundle -- --nocapture` が通過した。
  full verification も通過した。
...
- 2026-06-14 11:15 JST: B8-ARCH2b B8 Debug Bundle I/O Boundary Split を開始し、
  ...
  runtime attempt orchestration、report DTO、loader/import projection、helper process
  execution、modeled continuation state は親 module に残した。対象テスト
