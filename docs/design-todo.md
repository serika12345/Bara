...
- 2026-06-23 に `MachOExecutableImageMapping` が `GuestImageMappedBytes` value object を
  保持するようにした。
- 意図は B8 debug bundle と後続 runtime loader caller が mapping snapshot から mapped bytes
  source だけでなく payload value object も読めるようにし、debug DTO projection の理由で
  `MachOImage` / metadata 内部構造へ戻らない境界を強めること。
- `MachOImage::executable_mapping()` は `GuestImageMetadata` の mapped bytes value object を
  clone して snapshot に渡す。`MachOExecutableImageMapping::mapped_bytes()` で value object を
  参照でき、`mapped_bytes_source()` は既存 caller 用に残して source を value object から導出する。
...
- 2026-06-23 に `MachOExecutableImageSnapshot` を追加し、
  `MachOImage::executable_snapshot()` から Mach-O executable image snapshot を取得できるようにした。
- 意図は後続の import / relocation / symbol / unwind projection が mapping と metadata を別々の
  accessor から集めず、Mach-O executable image snapshot を単一の入口にできるようにし、
  B8-ARCH2 の image model 境界を強めること。
- `MachOExecutableImageSnapshot` は `MachOExecutableImageMapping` と
  `MachOExecutableImageMetadata` を保持し、payload primitive を直接公開するための新しい API は
  増やさない。
- これにより caller は mapping snapshot と metadata snapshot を同じ Mach-O specific boundary
  経由で扱える。B8 debug bundle の通常経路も executable image snapshot を入口にして existing
  `image_mapping` report を組み立て、mapped bytes source は metadata snapshot の value object から
  読む。existing B8 debug bundle behavior と `loader.plan.json` output は変えない。
- `bara-oracle` からの loader domain 抽出、entry extraction / load command interpretation、
  public Mach-O parser / resolver logic、import/fixup/symbol projection semantics の意味変更、
  Objective-C/AppKit helper process execution、modeled continuation state、runtime dispatcher は
  同じ PR では動かさない。
...
- 2026-06-27 に B8 debug bundle loader plan が `MachOEntryFunctionInput` から
  `MachOExecutableImageSnapshot` を一度作り、その snapshot を `image_mapping` projection へ
  渡すようにした。
- 意図は loader plan assembly が Mach-O executable image snapshot を単一の入口として扱い、
  debug DTO 側が entry input から image model を作り直さない境界にすること。
- `mach_o_executable_snapshot_from_entry_input` を `b8_debug_bundle::guest_image` に置き、
  `B8DebugGuestImageMappingReport::from_mach_o_snapshot` は borrowed snapshot から existing
  `image_mapping` JSON を組み立てる。JSON field 名、serde 値、existing B8 debug bundle
  behavior は変えない。
- `bara-oracle` からの loader domain 抽出、entry extraction / load command interpretation、
  public Mach-O parser / resolver logic、import/fixup/symbol projection semantics の意味変更、
  Objective-C/AppKit helper process execution、modeled continuation state、runtime dispatcher は
